...
class SmashEvent:
    fps: float
    origin_video_idx: int

    # Internal Smash Indices (Relative to the slice: 0 to N)
    start_frame_idx: int
    start_time_str: str
    end_frame_idx: int
    end_time_str: str
    ...
    # Absolute Origin Indices (Relative to the specific raw video)
    origin_frame_offset: int
    origin_start_frame_idx: int
    origin_start_time_str: str
    origin_end_frame_idx: int
    origin_end_time_str: str

    # Absolute Global Indices (Relative to the fully joined timeline)
    joined_frame_offset: int
    joined_start_frame_idx: int
    joined_start_time_str: str
...
def find_smashes(kin_metrics: list[FrameMetrics],
    ...
    for start_idx, end_idx in swing_blocks:
        ...
        if time_spans_included:
            if not any(start <= peak_time_sec <= end for start, end in time_spans_included): continue
        if time_spans_excluded:
            if any(start <= peak_time_sec <= end for start, end in time_spans_excluded): continue
        ...
        smashes.append(SmashEvent(
            fps=fps,
            origin_video_idx=origin_video_idx,

            # Smash-Relative indices
            start_frame_idx=0,
            start_time_str=_format_timestamp(0, fps),
            end_frame_idx=smash_length,
            end_time_str=_format_timestamp(smash_length, fps),
            peak_frame_idx=peak_idx - s_start,
            peak_time_str=_format_timestamp(peak_idx - s_start, fps),
            critical_start_frame_idx=best_dec_start - s_start,
            critical_start_time_str=_format_timestamp(best_dec_start - s_start, fps),
            critical_end_frame_idx=best_dec_end - s_start,
            critical_end_time_str=_format_timestamp(best_dec_end - s_start, fps),
            overhead_start_frame_idx=start_idx - s_start,
            overhead_start_time_str=_format_timestamp(start_idx - s_start, fps),
            overhead_end_frame_idx=end_idx - s_start,
            overhead_end_time_str=_format_timestamp(end_idx - s_start, fps),

            # Raw Video Object indices
            origin_frame_offset=s_start,
            origin_start_frame_idx=s_start,
            origin_start_time_str=_format_timestamp(s_start, fps),
            origin_end_frame_idx=s_end,
            origin_end_time_str=_format_timestamp(s_end, fps),

            # Placeholders for global timeline (populated post-extraction in app.py)
            joined_frame_offset=0,
            joined_start_frame_idx=0,
            joined_start_time_str="",
...
def plot_smash_kinematics(metrics: list[FrameMetrics],
    ...
    ang_accel = [m.upper_arm_angular_acceleration for m in metrics]
...
def _draw_double_arrow(img, start_pt, end_pt, color, thickness=2, scale=1.0):
    ...
    tip_width = 8.0 * scale
    if tip_width <= offset: tip_width = offset + (2.0 * scale)
...
def overlay_kinematic_assessment(input_video_path,
    ...
    # 1. Fast forward to isolated clip start frame natively without storing garbage frames
    frame_idx = 0
    ...
    summary_start_frame = float('inf')
    if smashes and use_joined_timeline:
        summary_start_frame = smashes[-1].joined_end_frame_idx
    ...
    while cap.isOpened():
        ...
        m_idx = frame_idx - start_frame
        if m_idx >= len(metrics): break

        m = metrics[m_idx]

        # --- DRAW KINEMATIC ARROWS ---
        if m.trunk_coord and m.trunk_speed > 20:
            start_pt = m.trunk_coord
            end_pt = (int(start_pt[0] + m.trunk_v[0] * scale_linear), int(start_pt[1] + m.trunk_v[1] * scale_linear))
        ...
        if m.elbow_coord and m.shoulder_coord:
            ...
            if m.elbow_speed > 30:
                end_pt_vel = (int(start_pt[0] + m.elbow_v[0] * scale_linear), int(start_pt[1] + m.elbow_v[1] * scale_linear))
        ...
        if m.wrist_coord and m.wrist_speed > 40:
            ...
            end_pt = (int(start_pt[0] + m.wrist_v[0] * scale_linear), int(start_pt[1] + m.wrist_v[1] * scale_linear))
        ...
        if m.grip_coord and m.grip_speed > 50:
            ...
            end_pt = (int(start_pt[0] + m.grip_v[0] * scale_linear), int(start_pt[1] + m.grip_v[1] * scale_linear))
